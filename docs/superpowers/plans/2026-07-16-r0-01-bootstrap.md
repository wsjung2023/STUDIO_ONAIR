...
Windows에서 Ninja 프리셋을 쓰려면 MSVC 환경이 필요하다. Developer PowerShell을 쓰거나 아래처럼 `vcvars64.bat`를 먼저 실행한다.
...
`cmake`(3.31.6)와 `ninja`(1.12.1)는 VS BuildTools에 번들되어 있으며 `vcvars64.bat` 실행 시 PATH에 올라온다.
...
```bash
git add src/project_store/JsonProjectStore.h src/project_store/JsonProjectStore.cpp src/project_store/CMakeLists.txt CMakeLists.txt tests/CMakeLists.txt tests/project_store/JsonProjectStoreTest.cpp
...
- [ ] **Step 1: OSS_BOM.csv에 두 의존성 추가**

`legal/OSS_BOM.csv` 끝에 두 줄 추가한다. 기존 칼럼 순서
(`component,purpose,planned_usage,license,linking_or_boundary,status,official_source,notes`)를 그대로 따른다:

```csv
GoogleTest,Unit test framework,Test executables only,BSD-3-Clause,Static link into test binaries only; never distributed,APPROVED,https://github.com/google/googletest,"Test-only dependency: not present in any shipped artifact. Pinned to v1.15.2 and fetched at configure time via CMake FetchContent"
...
두 라이선스 모두 `legal/LICENSE_POLICY.md`의 허용 기본군(MIT, BSD-3-Clause)에 있다.

GoogleTest의 `linking_or_boundary`에 배포되지 않는다고 명시하는 이유: 이 구분이 없으면 나중에 SBOM 생성기가 배포하지도 않는 것을 배포 목록에 올린다. R4의 "SBOM 자동 생성"과 "금지 라이선스 CI 검사"가 이 칼럼을 읽는다.
...
```bash
git add legal/OSS_BOM.csv .gitattributes .github/workflows/ci.yml README.md tests/README.md
git commit -m "chore: record new dependencies, normalise line endings, add CI

Adds GoogleTest (BSD-3) and nlohmann/json (MIT) to OSS_BOM.csv as CLAUDE.md 7
requires. Both are in the LICENSE_POLICY 허용 기본군. GoogleTest's boundary
column says test-only and not distributed: without that distinction the SBOM
generator in R4 would report us shipping something we never ship.
