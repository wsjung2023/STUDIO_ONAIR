...
TEST(FrameConversionTest, TwoHoursAtDropFrameRateDoesNotDrift) {
    ...
    // 431568 frames is the two hour soak ARCHITECTURE.md 5.3 sets targets for.
    // Exactly: 431568 * 1001 / 60000 s = 7199.9928 s = 7199992800000 ns. The
    // division is exact here, so this asserts the arithmetic has not drifted by
    // even one nanosecond over the full take.
...
TimestampNs frameToTimestamp(std::int64_t frameIndex, FrameRate rate) noexcept {
    ...
    // multiply happens before the divide and no precision is lost early.
    //
    // Rounds UP, and that is load-bearing rather than a taste call.
    // timestampToFrame truncates, so the round trip only holds if this lands at
    // or after the frame's exact presentation time. Truncating here instead
    // sends frame 1 at 60fps to 16666666ns, which maps back to frame 0 - and
    // frame 59 back to 58. Rounding up is what makes
    // timestampToFrame(frameToTimestamp(n)) == n hold for every n.
    //
    ...
    // ~4.3e17 - an order of magnitude below int64 max (~9.2e18).
    const std::int64_t scaled = frameIndex * rate.denominator() * 1'000'000'000LL;
    const std::int64_t ns = -(-scaled / rate.numerator());
...
ProjectManifest makeValidManifest() {
    ...
    return ProjectManifest{
        ...
        // std::string{...} is required, not decoration. kDatabaseFileName is a
        // string_view and std::string's converting constructor from one is
        // explicit, so `= kDatabaseFileName` is copy-initialisation and does not
        // consider it. MSVC rejects it with C2440.
...
public:
    ...
    /// Hands an audio block to the recorder. ok() means the same thing: the
    /// recorder is healthy.
    ///
    /// But audio is not video here. Audio is the master stream for
    /// synchronisation (ARCHITECTURE.md 5.2) and "오디오 손실은 조용히 무시하지
    /// 않는다" (CLAUDE.md 5), so a recorder that cannot keep up with audio
    /// returns an error rather than dropping and counting. That asymmetry is
    /// why RecorderStats has framesDropped but deliberately no
    /// blocksDropped - a counter for it would say silent audio loss is a state
    /// we expect to be in.
    [[nodiscard]] virtual creator::core::Result<void> accept(
...
//
// Assignable, NOT constructible. is_copy_constructible_v is false for ANY
// abstract class - abstractness alone disqualifies construction - so it would
// hold here whether or not we deleted anything, and would pass unchanged if
// someone removed the = delete lines. The assignment traits work through
// references, which abstractness does not affect, so they actually track the
// = delete.
...
static_assert(!std::is_copy_assignable_v<IPullCaptureSource>);
static_assert(!std::is_move_assignable_v<IPullCaptureSource>);
...
/// Reproduces frameToTimestamp's ceiling formula independently, so the
/// expectation is derived rather than copied from the implementation.
///
/// Must round UP, not truncate. frameToTimestamp rounds up because
/// timestampToFrame floors, and the round trip only holds if the timestamp
/// lands at or after the frame's exact presentation time (see core/Timebase.h).
/// Truncating here instead is off by 1ns on 40 of the 61 frames below - it was
/// wrong in an earlier draft of this plan and the tests caught it.
[[nodiscard]] std::int64_t expectedNs(std::int64_t frame, std::int64_t num, std::int64_t den) {
    const std::int64_t scaled = frame * den * 1'000'000'000LL;
    return -(-scaled / num);
}
...
TEST(FakeCaptureSourceTest, ProducesSixtyFpsTimestamps) {
    ...
    // Default config is 60/1, so frames land 1/60s apart and frame 60 is
    // exactly one second in. No clock is read: the timestamp is computed from
    // the frame index.
...
/// Where a take's timeline starts.
///
/// Zero, NOT ProjectClock::now(). The source computes frame timestamps from the
/// frame index (frameToTimestamp), so its first frame is at 0 - anchoring the
/// session at a steady-clock reading instead puts the session and its frames on
/// two unrelated numeric scales. The recorder then never sees a frame far
/// enough past segmentStart_ to close a segment, and the take reports 00:00:00.
///
/// R0-03's real sources timestamp against ProjectClock, and this anchor becomes
/// the capture start reading rather than a constant.
constexpr core::TimestampNs kTakeOrigin{};
