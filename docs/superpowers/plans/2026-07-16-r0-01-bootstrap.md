...
// The default pixel format is checked at runtime below, not here. VideoFrame{}
// cannot appear in a constant expression: its implicit destructor is constexpr
// only if every subobject's is, and std::shared_ptr's is not. MSVC rejects it
// with C2131 naming VideoFrame::~VideoFrame.
...
TEST(MediaTypesTest, DefaultFrameIsEmpty) {
    ...
    EXPECT_EQ(frame.colorSpace, ColorSpace::Rec709Sdr);
}
