...
이 태스크에는 새 테스트를 넣지 않는다. 동작이 없는 순수 데이터 선언이고, 검증할 불변식이 아직 없다. 값이 생기는 시점(R0-03에서 실제 프레임이 들어올 때)에 테스트가 따라온다.
...
cmake --build --preset windows-debug
```
Expected: 성공. `cs_media` 타깃이 만들어지고 Qt-free 가드를 통과한다.

- [ ] **Step 5: 커밋**

```bash
git add src/media/MediaTypes.h src/media/CMakeLists.txt CMakeLists.txt
...
Documents why platformHandle is a type-erased shared_ptr and why ColorSpace
has exactly one enumerator."
```
...
이 태스크에는 테스트를 넣지 않는다. 순수 추상 인터페이스라 검사할 동작이 없고, 실제 검증은 Task 10의 fake 구현이 한다.
...
ctest --preset windows-debug
```
Expected: 세 타깃이 만들어지고 Qt-free 가드를 통과, 기존 테스트 전부 PASS.

- [ ] **Step 9: 커밋**

```bash
git add src/capture/ICaptureSource.h src/capture/CMakeLists.txt src/recorder/IRecorder.h src/recorder/CMakeLists.txt src/project_store/IProjectStore.h src/project_store/CMakeLists.txt CMakeLists.txt
...
All three ports delete copy and move: implementations own OS handles, and a
copied source double-closes them. They are always held by unique_ptr.
...
**Interfaces:**
- Consumes: `ICaptureSource`, `CaptureConfig`, `CaptureStats`, `IRecorder`, `RecorderConfig`, `RecorderStats` (Task 8), `FrameRate`, `frameToTimestamp` (Task 2), `RecordingSession`, `SegmentInfo` (Task 7), `VideoFrame` (Task 5)
- Produces:
  - 타깃 `cs_fakes` (DEPENDS `cs_capture`, `cs_recorder`)
  - `class creator::fakes::FakeCaptureSource` — `FakeCaptureSource(domain::SourceId, std::string displayName)`, `ICaptureSource` 전부 + `core::Result<media::VideoFrame> tick()`
...
/// An ICaptureSource that emits timestamps and nothing else.
///
...
class FakeCaptureSource final : public creator::capture::ICaptureSource {
public:
    ...
    [[nodiscard]] creator::core::Result<creator::media::VideoFrame> tick();
...
**Interfaces:**
- Consumes: `ICaptureSource`, `CaptureConfig`, `IRecorder`, `RecorderConfig` (Task 8), `FakeCaptureSource`, `FakeRecorder` (Task 10), `SessionId`, `SourceId` (Task 4), `RecordingSession` (Task 7)
- Produces:
  ...
  - `class creator::app::StudioController : public QObject`
    - 기본 생성자 (fake를 스스로 조립) / DI 생성자 `StudioController(std::unique_ptr<ICaptureSource>, std::unique_ptr<IRecorder>, QObject*)`
...
`QTimer`가 `onCaptureTick()`을 호출해 fake에서 프레임을 뽑는다. `onCaptureTick()`이 public slot이라 테스트는 타이머 없이 직접 호출해 결정적으로 검사한다. R0-03에서 이 슬롯은 실제 캡처 콜백으로 대체된다.
...
///
/// For this bootstrap the controller drives a FakeCaptureSource with a QTimer:
/// there is no real capture yet, and the point is to prove that Record and Stop
/// move a session through the ports. R0-03 replaces the timer with real capture
/// callbacks; nothing above this class changes when it does.
...
public:
    ...
    StudioController(std::unique_ptr<creator::capture::ICaptureSource> source,
...
StudioController::StudioController(std::unique_ptr<capture::ICaptureSource> source,
...
void StudioController::onCaptureTick() {
    ...
    // Only FakeCaptureSource produces frames on demand. Real sources push them,
    // which is why this whole slot disappears in R0-03.
    auto* fake = dynamic_cast<fakes::FakeCaptureSource*>(source_.get());
    if (fake == nullptr) {
        return;
    }

    auto frame = fake->tick();
    if (!frame.hasValue()) {
...
`onCaptureTick()`의 `dynamic_cast`는 임시 배선이다. R0-03에서 실제 소스가 프레임을 push하면 이 슬롯 자체가 사라진다. 주석에 그렇게 적어 둔다 — 지금 이걸 우아하게 만들려고 추상화를 하나 더 얹으면, 두 달 뒤 지울 코드에 설계를 낭비하는 것이다.
...
The dynamic_cast in it is temporary scaffolding: real sources push frames, so
the slot disappears in R0-03 rather than growing an abstraction for code with
two months to live.
