...
Callbacks for a source are serialized. `stop()` is an idempotent barrier: after
it returns, that source will not make another sink call.
...
4. derive current pixel dimensions from the buffer, not stale selection data;
5. retain the buffer in `platformHandle` with a matching release deleter;
6. publish without copying pixel bytes.

`stream:didStopWithError:` translates user stop/target removal/permission and
unknown native errors into product errors. It emits one terminal error and makes
subsequent `stop()` safe.
...
uses aspect-fit letterboxing and reads dimensions from every frame, so a window
resize requires no stream restart.
...
- actual width, height, fps, and dropped/replaced counters;
