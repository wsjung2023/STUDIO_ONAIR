...
SQLite 공식 amalgamation 3.53.3을 URL과 SHA3-256으로 고정해 정적 C 라이브러리로
빌드한다. 프로젝트 언어에 C를 추가하고 시스템별 우연한 SQLite 설치에 의존하지
않는다. BOM에는 Public Domain, 버전, 원본 URL, 해시를 기록한다.
...
사용자는 부모 폴더와 프로젝트 이름을 고른다. application 계층이 파일명에
`.cstudio`가 없으면 붙이고 다음 순서로 생성한다.
...
- macOS/POSIX: `fsync` 후 `rename`, 가능한 플랫폼에서는 부모 디렉터리도 `fsync`
...
동일 버전의 checksum이 빌드와 다르면 DB를 수정하지 않고 실패한다. DB의 최고 버전이
앱이 아는 최고 버전보다 크면 `UnsupportedVersion`이다. rollback migration은 만들지
않는다.
...
- 새 녹화 프로젝트: 이름과 부모 폴더를 선택해 패키지를 만든 뒤 Studio로 이동
- 기존 프로젝트 열기: 폴더를 선택해 검증·마이그레이션·복구 스캔
- 최근 프로젝트: 성공적으로 생성·연 프로젝트 경로
- 복구 가능한 녹화: 등록된 최근 프로젝트에서 발견한 recovery candidate

최근 프로젝트 registry는 application 설정 영역에 저장하고 작업 스레드에서 읽고 쓴다.
프로젝트 내부 데이터의 source of truth는 아니다. registry에서 경로가 사라졌다고
프로젝트나 미디어를 삭제하지 않는다.
...
상세 계획은 별도 문서에서 TDD 단위로 나눈다. 구현은 대략 다음 의존 순서를 지킨다.
