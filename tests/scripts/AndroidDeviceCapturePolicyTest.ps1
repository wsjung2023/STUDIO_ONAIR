...
$checks = @(
    ...
    @{ Path = $backend; Pattern = 'AndroidDeviceSession'; Name = 'generation-scoped device lifecycle' },
    @{ Path = $backend; Pattern = 'failed\(std::uint64_t generation'; Name = 'generation-scoped native failures' },
    ...
    @{ Path = $activity; Pattern = 'nativeCameraStopped'; Name = 'camera release completion' },
    @{ Path = $activity; Pattern = 'nativeMicrophoneStopped'; Name = 'microphone release completion' },
    @{ Path = $activity; Pattern = 'nativeSystemAudioStopped'; Name = 'playback-audio release completion' },
    @{ Path = $activity; Pattern = 'worker\.join\(\)'; Name = 'AudioRecord worker join barrier' },
